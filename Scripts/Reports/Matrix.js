...
// If you don't want a  new spreadsheet to be created
// each time you run the script, enter a spreadsheet-URL here.
var SPREADSHEET_URL = '';

// Avaiable KPIs: http://goo.gl/mdtDdh (green ones)
var KPI_TO_LOOK_AT = ['SearchImpressionShare', 'Clicks'];
...
function createSpreadsheet(data) {
    var spreadsheetName = 'Matrix report: ' + new Date().toLocaleDateString();
    var rowLabels = [
    ...
    var spreadsheet = (SPREADSHEET_URL) ? SpreadsheetApp.openByUrl(SPREADSHEET_URL) : SpreadsheetApp.create(spreadsheetName);

    KPI_TO_LOOK_AT.forEach(function(columnName) {

        var sheet = (SPREADSHEET_URL) ? spreadsheet.getSheetByName(columnName) : spreadsheet.insertSheet(columnName);

        if(sheet == null) { var sheet = spreadsheet.insertSheet(columnName); }
    ...
    if( ! SPREADSHEET_URL) spreadsheet.deleteSheet(spreadsheet.getSheets()[0]);
